...
class BinaryHeap
{
    ...
    vector<AvlNode*> pointers; // The heap array
...
void insert(AvlNode * & t, int p )// insert node pointer and priority into heap vector
{
    if((pointers.size()<pointers.capacity())&&(priorities.size()<priorities.capacity()))
    {
        pointers.pushback(t);
        priorities.pushback(p);
    }
...
void deletemin()// delete minimum from vectors (element at index 0)
{
...
AvlNode * & findmin()// return highest (smallest) priority reference to object
{
    if((pointers.size()>0)&&(priorities.size()>0))
    {
...
bool isempty()// check if heap vector sizes= 0
{
    if((pointers.size()==0)&&(priorities.size()==0))
    {
        return true;
...
int size()// return size of heap vectors
{
...
void makeempty()// delete minimum from vectors (element at index 0)
{
    if((pointers.size()>0)&&(priorities.size()>0))
    {
        pointers.clear();
...
void updatepriority()// delete minimum from vectors (element at index 0)
{
    if((pointers.size()>0)&&(priorities.size()>0))
    {
        pointers.clear();
...
*****************************************************/
    ...
    int depth( const Comparable & x, PQNode *t ) const
    {
        ...
        else if( x < t->element )
        {
            ...
            return depth( x, t->left )+1;#include <algorithm>
#include <iostream> 
#include <vector>
#include "AvlTree.h"
using namespace std;
